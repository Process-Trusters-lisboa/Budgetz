...
function Dashboard() {
  ...
  const handleSaveEdit = async (docid) => {
    ...
    try {
      await axios.patch(
        `https://firestore.googleapis.com/v1/projects/budgetz-7b9d9/databases/(default)/documents/newusers/user1/${expense.category}/${docid}`,
        {
          fields: { amount: { integerValue: parsedValue } },
        }
      );
      const updatedExpensesList = expensesList.map((expense) =>
        expense.docid === docid ? { ...expense, amount: parsedValue } : expense
      );
      setExpensesList(updatedExpensesList);
      setExpensesData(
        expensesData.map((item) =>
          item.name === expense.category
            ? { ...item, value: item.value - expense.amount + parsedValue }
            : item
        )
      );
  ...
  const handleDeleteExpense = async (docId) => {
    try {
      ...
      await axios.delete(
        `https://firestore.googleapis.com/v1/projects/budgetz-7b9d9/databases/(default)/documents/newusers/user1/${deletedExpense.category}/${docId}`
      );
      setExpensesList(
        expensesList.filter((expense) => expense.docid !== docId)
      );
      setExpensesData(
        expensesData.map((item) =>
        ...
        <div className="mb-6">
          ...
          <div className="w-full h-[300px] sm:h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={[{ name: "Remaining Balance", value: remainingBalance }]}
        ...
        <div className="mt-6">
          ...
          <ul>
            {expensesList.map((expense, index) => (
              <li
                key={index}
                className="flex flex-col sm:flex-row items-center justify-between mb-2 p-2 border-b border-gray-200"
              >
